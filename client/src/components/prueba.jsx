...
export default function Prueba() {
    ...
    return (
        <>
            ...
            <aside className={`fixed inset-y-0 left-0 z-50 bg-white w-64 overflow-y-auto transition-transform duration-300 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <nav className="h-full flex flex-col bg-white border-r shadow-sm">
                    ...
                    <div className="p-4 pb-2 flex justify-between items-center">
                        ...
                        <Tooltip content="Add new Project" className='ml-1 z-20' placement="right">
            ...
            <Tooltip content="Projects" className='ml-1 z-20'>
            <button onClick={toggleSidebar} className={`fixed top-2.5 left-4 p-1.5 rounded-lg  hover:bg-indigo-200 ${isSidebarOpen ? 'hidden' : ''}`}>
                <AlignJustify />
            </button>
            </Tooltip>
        </>
